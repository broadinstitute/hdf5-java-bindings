...
public final class HDF5LibraryUnitTest {
    ...
    public void loadLibrary(){
        if (!new HDF5Library().load(null)){
            throw new SkipException("hdf5 not supported");
        }
    }

    private static final File TEST_RESOURCE_DIR = new File("src/test/resources/org/broadinstitute/hdf5");
    public static final File TEST_PON = new File(TEST_RESOURCE_DIR,"test_creation_of_panel.pon");
    ...
    public void testCreateHDF5File() {
        final File testFile = Utils.createTempFile("hdf5", ".hd5");
        testFile.delete();
